    ...
    def add(self, embeddings: np.ndarray, paths: List[str]) -> List[int]:
        ...
        for path in paths:
            ...
            if path in self.path_to_id:
                ids.append(self.path_to_id[path])
                continue
            idx = self._next_id
        ...
        # FAISS 按顺序添加，ID 为 0..N-1 的自增
        # 注意：这里我们用位置 ID 对应内部 ID，需要确保一致
        # 对于新增的向量，取最后 len(new) 个
        new_mask = [i for i, p in enumerate(paths) if p not in self.path_to_id or self.path_to_id[p] >= self.index.ntotal]
        # 简化：直接全部添加，去重由上层保证
        self.index.add(embeddings)
        logger.info("添加 %d 条向量，索引总量=%d", len(embeddings), self.index.ntotal)
        return ids
